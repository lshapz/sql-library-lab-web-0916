...
def select_value_and_count_of_most_prolific_species
  "SELECT species, COUNT(*) FROM characters GROUP BY species ORDER BY COUNT(characters.species) DESC LIMIT 1"
end
...
def select_series_title_with_most_human_characters
  "SELECT series.title FROM series JOIN characters on series.id = characters.id ORDER BY characters.species = 'human' LIMIT 1"
end
